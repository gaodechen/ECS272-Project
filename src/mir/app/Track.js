...
export default class Track {
  ...
  process() {
    ...
    emitter.emit("featuresProcessed");
  ...
  computeHarmonicStructure() {
    ...
    emitter.on("harmonicStructure", this.harmonicStructureListener);
  ...
  deselect() {
    if (this.eventListenerSet) {
      emitter.off("harmonicStructure", this.harmonicStructureListener);
      emitter.off("tsneReady", this.harmonicStructureListener);
  ...
  updateTimbreVis(timbreSliders) {
    ...
    workers
      .updateTimbreGraphVis(
        this.features.downSampledTimbre,
        timbreSliders,
        this.features.sampleDuration
      )
      .then((result) => {
  ...
  cluster() {
    workers
      .cluster({ features: this.smoothedTimbre, minK: 2, maxK: 10, tries: 10 })
      .then((result) => {
        ...
        this.clusterSections[this.clusterSections.length - 1].end =
          result.length;
  ...
  tsne() {
    ...
    emitter.on("tsneReady", this.tsneListener);
    this.eventListenerSet = true;
  }
  tsneListener = (result) => {
  ...
  placeMarker(time) {
    ...
    const alreadyPlacedMarkerIndex = this.markers.findIndex(
    ...
    const color = d3
      .color(vis.goldenRatioCategoricalColor(this.markersCreated, 0))
      .formatHex();
  ...
  reload() {
    ...
    if (this.hasVisualization()) {
      window.setTimeout(() => emitter.emit("readyForPrototypeVis"), 0);
    }
  }
