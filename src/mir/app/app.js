...
export async function initialize() {
  ...
  if (!firstTrack) {
    firstTrack = (await spotify.getPlaylistTracks("34wIIW2zm3vw5ZRqO8dHi0"))
      .items[0].track;
  }
...
export function loadAllTracks() {
  if (localStorage.allTracks) {
    ...
    allTracksArray.forEach((trackArray) => {
      const track = Track.createWithAnalysis(
        trackArray[1].trackData,
        trackArray[1].analysisData
      );
