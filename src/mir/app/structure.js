...
export function computeStructureCandidates(
  ...
  const scoreMatrixBuffer =
    pathExtraction.createScoreMatrixBuffer(sampleAmount);
  const candidates = [];
...
function mergeOverlap(sections) {
  ...
  sections.forEach((section) => {
    ...
    if (hasToMerge) {
      mergedSections[indexToMergeWith] = mergeSection(
        mergedSections[indexToMergeWith],
        section
      );
    } else {
...
function mergeSection(sectionA, sectionB) {
  ...
  return new Section({
    ...
    groupID: sectionA.groupID,
    confidence: sectionA.confidence,
  });
}
...
function getFreeRangesNonOverlapping(
  structureSections,
  trackEnd,
  smallestAllowedSize
) {
...
export function MDSColorGivenDistanceMatrix(
  ...
  const coloredSegments = [];
  const MdsCoordinates = mds.getMDSCoordinates(
    distanceMatrix,
    coloringStrategy
  );
